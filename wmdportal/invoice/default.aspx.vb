...
Public Class _default10
    ...
    Protected Sub PaymentSelection_CheckedChanged(sender As Object, e As EventArgs)
        For Each row As GridViewRow In PaymentGridView.Rows
            ...
            If CheckedBox IsNot Nothing AndAlso CheckedBox.Checked Then
                ...
                Dim PaymentDescription As String = PaymentDescription + row.Cells(3).Text + ","
                ''FilterBills.Text = PaymentDescription
                Session.Add("PaymentDescription", PaymentDescription)
                'MessageBox.Show(Me, Session("PaymentDescription"))
            End If
                            ...
                            Using da As New MySqlDataAdapter(cn)
                                ...
                                If Session("TotalBalance") > 0 Then
                                    ...
                                    If Session("TotalBalance") = AmountDue Or Session("TotalBalance") > AmountDue Then
                                        ...
                                        Dim cmd As New MySqlCommand("UPDATE payment SET accStatus = @AccountStatus, amountPaid = @AmountPaid, paymentMode = @Mode, paymentDate = @Date, paymentTime = @Time, paymentStatus = @Status, orderID = @InvoiceNumber, orderID = @WalletBallanceAfterPayment WHERE companyID = @CompanyID AND transCode = @ReferenceCode")
                                        cmd.Connection = conn
                                        ...
                                        cmd.Parameters.AddWithValue("@WalletBalanceAfterPayment", WalletBalanceAfterDeduction)
                                        cmd.ExecuteNonQuery()
    ...
    Protected Sub OnInvoiceSelectedIndexChanged(sender As Object, e As EventArgs)
        ...
        Try
            ...
            Using cn As MySqlCommand = New MySqlCommand(db, conn)
                Using da As New MySqlDataAdapter(cn)
                    ...
                    InvoiceID.Text = ds.Tables(0).Rows(0).Item("orderID").ToString
                    InvoiceDate.Text = ds.Tables(0).Rows(0).Item("invoiceDate").ToString & "   " & ds.Tables(0).Rows(0).Item("invoiceTime").ToString
                    InvoiceCompanyAddress.Text = Session("UserLoginCompanyName")
                    InvoiceUsername.Text = Session("UserLoginName")
                    InvoiceCompanyAddress.Text = Session("UserLoginCompanyAddress")
                    InvoiceTotalAmount.Text = "   " & FormatNumber(ds.Tables(0).Rows(0).Item("InvoiceTotalAmount").ToString, 2, TriState.False, , TriState.True)
        ...
        End Try
        Response.AppendHeader("Refresh", "0;url=#ViewInvoice")
    End Sub
