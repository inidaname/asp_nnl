...
Partial Class ISPregisterdevice
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        Try
            ...
            If String.IsNullOrEmpty(Session("ispCompID")) = True Then
                Try
            ...
            txtAccountID.Text = Session("ispCompID")
    ...
    Private Sub filldb(ByVal w As String)
        Dim ds As DataSet = GenTool.DataSetData("select sysID as ID,(select DeviceType from maindevicesection where sysID=deviceregistration.TypeOfDevice) as 'Instrument Type',actualMeasure as Capacity,modelNumber as Model,serialNumber as Serial,manufactureNumber as Manu_Number,deviceAmount as Fee,feeID as FID,BarCode,date_format(regDate,'%D-%M-%Y') as TransDate from deviceregistration where  AccountID=" & Val(Session("ispCompID")) & " " & w)
        Try
            Dim fPaid As String = GenTool.getSingleValue("select sum(deviceAmount) from deviceregistration where vStatus=1 AND AccountID=" & Val(Session("ispCompID")) & " " & w)
...
Reapeat:        txtBarCode.Text = Strings.Left(GenTool.GenerateUniqueCode & GenTool.GenerateRNDCode, 8)
            ...
            If RadioButton3.Checked = False AndAlso RadioButton2.Checked = False AndAlso RadioButton1.Checked = False Then
                msgtext = "At least one certificate options must be selected"
                Return
            End If
        ...
        If GenTool.HasDatasetAnyRecord(dsFillCompany) = False Then
            With dsFillCompany.Tables(0).Rows(0)
                ...
                If RadioButton3.Checked = True Then
                    ...
                    Footer = "NOTE: This is a confidential information, please discard if it is received in error."
