...
Partial Class exportpermit
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        Try
            ...
            If IsPostBack = False Then
                ...
                'If String.IsNullOrEmpty(msgAlert) = False Then
                '    Dim folderPath As String = "exportpermitefee.aspx"
                '    ScriptManager.RegisterStartupScript(Page, GetType(Page), "OpenWindow", "window.open('" & folderPath & "','_blank','directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,dependent,resizable=yes,scrollbars=yes,top=0,width=800,height=800');", True)
                'End If
    ...
    Protected Sub btnPreview_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPreview.Click
        ...
        Try
            ...
            txtpassportname7.Text = IIf(String.IsNullOrEmpty(Session("fileLetterPicture7")) = True, "", Session("fileLetterPicture7"))
            txtpassportname8.Text = IIf(String.IsNullOrEmpty(Session("fileLetterPicture8")) = True, "", Session("fileLetterPicture8"))
            ...
            If chkDeclare.Checked = False Then
                msgtext = "You have not accepted our declaration"
                Return
            End If
            ...
            Else
                Dim fs As String = "finalQuantity;TerminalOperator;NoneTerminalOperator;exportpermitName;exporterName;productnameforcast;periodCoveredFrom;periodCoveredTo;Measure;quantity;exportPort;amtPerBarrelUS;TotalamtUS;DPRApplication;EvidenceOfMonitoringFees"
                msgtext = GenTool.generateSQL(Panel1, FN, FV, FUpdate, fs)
                FN &= ",TerminalOperator,NoneTerminalOperator"
                FV &= "," & IIf(RadioButton3.Checked = True, "1", "0") & "," & IIf(RadioButton4.Checked = True, "1", "0")
            End If
            ...
            If String.IsNullOrEmpty(Session("fileLetterPicture8")) = True Then
                msgtext = "Attach Evidence of Payment of Monitoring Fees"
                Return
            End If
...
"</html>"
    ...
    '    fileLetterPicture7.Visible = CheckBox7.Checked
    '    fileLetterPicture8.Visible = CheckBox8.Checked
    'End Sub
    ...
    Protected Sub btnPreview0_Click(sender As Object, e As System.EventArgs) Handles btnPreview0.Click
        ...
        Try
            ...
            Dim fs As String = "TerminalOperator;NoneTerminalOperator;CertificateOfIncor;CompanyArticleMemo;LicenseByDPR;WeightMeters;OriginalBank;TaxClearance;docFiledTitle;docfilledBy;emaildocsigned;companyID;txtpassportname1;txtpassportname2;txtpassportname3;txtpassportname4;txtpassportname5;txtpassportname6;DPRApplication;EvidenceOfMonitoringFees;CoverLetterName8;CoverLetterName7"
    ...
    Sub CompleteUpload()
        ...
        Try
            If fileLetterPicture7.HasFile = True Then
                Dim extN As String = Guid.NewGuid.ToString & IO.Path.GetExtension(fileLetterPicture7.FileName)
                Dim fil As String = pathdoclink & "coverLetter/" & extN
                fileLetterPicture7.SaveAs(fil)
                Session.Add("fileLetterPicture7", extN)
            End If
        Catch ex As Exception
            MessageBox.Show(Panel1, ex.Message)
        End Try
        ...
        Try
            If fileLetterPicture8.HasFile = True Then
                Dim extN As String = Guid.NewGuid.ToString & IO.Path.GetExtension(fileLetterPicture8.FileName)
                Dim fil As String = pathdoclink & "coverLetter/" & extN
                fileLetterPicture8.SaveAs(fil)
                Session.Add("fileLetterPicture8", extN)
            End If
        Catch ex As Exception
            MessageBox.Show(Panel1, ex.Message)
        End Try
    ...
    Protected Sub fileStuPicture_UploadedComplete(sender As Object, e As System.EventArgs)
        Try
            If fileLetterPicture1.HasFile = True Then
                Dim extN As String = Guid.NewGuid.ToString & IO.Path.GetExtension(fileLetterPicture1.FileName)
                Dim fil As String = pathdoclink & "coverLetter/" & extN
    ...
    Protected Sub fileLetterPicture2_UploadedComplete(sender As Object, e As AjaxControlToolkit.AsyncFileUploadEventArgs)
        Try
            If fileLetterPicture2.HasFile = True Then
                Dim extN As String = Guid.NewGuid.ToString & IO.Path.GetExtension(fileLetterPicture2.FileName)
                Dim fil As String = pathdoclink & "coverLetter/" & extN
    ...
    Protected Sub fileLetterPicture3_UploadedComplete(sender As Object, e As AjaxControlToolkit.AsyncFileUploadEventArgs)
        Try
            If fileLetterPicture3.HasFile = True Then
                Dim extN As String = Guid.NewGuid.ToString & IO.Path.GetExtension(fileLetterPicture3.FileName)
                Dim fil As String = pathdoclink & "coverLetter/" & extN
    ...
    Protected Sub fileLetterPicture4_UploadedComplete(sender As Object, e As AjaxControlToolkit.AsyncFileUploadEventArgs)
        Try
            If fileLetterPicture4.HasFile = True Then
                Dim extN As String = Guid.NewGuid.ToString & IO.Path.GetExtension(fileLetterPicture4.FileName)
                Dim fil As String = pathdoclink & "coverLetter/" & extN
    ...
    Protected Sub fileLetterPicture5_UploadedComplete(sender As Object, e As AjaxControlToolkit.AsyncFileUploadEventArgs)
        Try
            If fileLetterPicture5.HasFile = True Then
                Dim extN As String = Guid.NewGuid.ToString & IO.Path.GetExtension(fileLetterPicture5.FileName)
                Dim fil As String = pathdoclink & "coverLetter/" & extN
    ...
    Protected Sub fileLetterPicture6_UploadedComplete(sender As Object, e As AjaxControlToolkit.AsyncFileUploadEventArgs)
        Try
            If fileLetterPicture6.HasFile = True Then
                Dim extN As String = Guid.NewGuid.ToString & IO.Path.GetExtension(fileLetterPicture6.FileName)
                Dim fil As String = pathdoclink & "coverLetter/" & extN
    ...
    Protected Sub btnUpload_Click(sender As Object, e As System.EventArgs) Handles btnUpload.Click
        CompleteUpload()
        MessageBox.Show(Panel1, "Uploaded successfully")
        Session.Add("isupload", "Yes")
    End Sub
