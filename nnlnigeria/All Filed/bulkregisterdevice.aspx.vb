    ...
    Private Sub saveData()
        ...
        Try
            ...
            For k As Int32 = 0 To dsMain.Tables(0).Rows.Count - 1
                ...
                Dim FNExtra As String = ",modelNumber,serialNumber,manufactureNumber,DeviceModelName,Location,barCode"
                Dim FVExtra As String = ""

                Dim barCode As String = ""

Reapeat:        barCode = Strings.Left(GenTool.GenerateUniqueCode & GenTool.GenerateRNDCode, 8)
                If GenTool.HasRows("SELECT barCode from deviceregistration where barCode=" & GenTool.addbackslash(barCode)) = True Then GoTo Reapeat




                With dsMain.Tables(0).Rows(k)
                    FVExtra = "," & GenTool.addbackslash(.Item(1).ToString) & "," & GenTool.addbackslash(.Item(2).ToString) & _
                        "," & GenTool.addbackslash(.Item(3).ToString) & "," & GenTool.addbackslash(.Item(4).ToString) & "," & _
                          GenTool.addbackslash(.Item(5).ToString) & "," & GenTool.addbackslash(barCode)
                    serialNo = .Item(2).ToString
                End With

                sql = "Insert into deviceregistration(" & FN & FNExtra & ") Values (" & FV & FVExtra & ")"
                        ...
                        Try
                            ...
                            With dsFillCompany.Tables(0).Rows(0)
                                sql = "Update deviceregistration set companyName=" & GenTool.addbackslash(.Item(0)) & _
                                    ...
                                    ",companywebsite=" & GenTool.addbackslash(.Item(7)) & " where AccountID=" & Val(Session("ID")) & " AND serialNumber=" & GenTool.addbackslash(serialNo)
                            End With
            ...
            grd0.DataSource = Nothing
            grd0.DataBind()
            msgtext = "Batch Instrument registered successfully"
    ...
    Protected Sub fileStuPicture_Load(sender As Object, e As System.EventArgs) Handles fileStuPicture.UploadedComplete
        Try
            If fileStuPicture.HasFile = True Then
                Dim isize As Long = fileStuPicture.FileBytes.Length / 1024

                Dim Ok As Boolean = genTool.checkPhotoExtension(IO.Path.GetExtension(fileStuPicture.FileName))
                If Ok = False Then
                    MessageBox.Show(Panel1, "You have not selected excel file")
                    Return
                End If

                Dim extN As String = Guid.NewGuid.ToString & IO.Path.GetExtension(fileStuPicture.FileName)
                Dim fil As String = MapPath("~/dumps/" & extN)
                fileStuPicture.SaveAs(fil)
                Session.Add("extS", MapPath("~/dumps/" & extN))
            End If
        Catch ex As Exception
        End Try
    End Sub
    ...
    Protected Sub btnExtractData_Click(sender As Object, e As System.EventArgs) Handles btnExtractData.Click
        Try
            Dim dsMain1 As DataSet = GenTool.ImportExcelXLS(Session("extS"), True)
            If GenTool.HasDatasetRecords(dsMain1) = False Then
                MessageBox.Show(Panel2, "Excel format is not in correct order,please contact our customer care")
                Return
            End If

            If dsMain1.Tables(0).Rows.Count < 6 Then
                MessageBox.Show(Panel2, "Number of columns must be 6 and they are ; SN,MODEL NO.,SERIAL/TAG NO.,MANUFACTURER'S NAME,INSTRUMENTS' MODEL NAME,LOCATION")
                Return
            End If

            dsMain = GenTool.DataSetData("select sysID as SN,'MODEL NO.','SERIAL/TAG NO.','MANUFACTURERS NAME','INSTRUMENTS MODEL NAME','LOCALTION' from emailsubs limit 0")
            dsMain.Tables(0).PrimaryKey = New DataColumn() {dsMain.Tables(0).Columns("SN")}

            For k As Int32 = 0 To dsMain1.Tables(0).Rows.Count - 1
                With dsMain1.Tables(0).Rows(k)
                    Dim dtRow As DataRow = dsMain.Tables(0).NewRow
                    dtRow.BeginEdit()
                    dtRow("SN") = .Item(0).ToString
                    dtRow("MODEL NO.") = .Item(1).ToString
                    dtRow("SERIAL/TAG NO.") = .Item(2).ToString
                    dtRow("MANUFACTURERS NAME") = .Item(3).ToString
                    dtRow("INSTRUMENTS MODEL NAME") = .Item(4).ToString
                    dtRow("LOCALTION") = .Item(5).ToString
                    dtRow.EndEdit()
                    dsMain.Tables(0).Rows.Add(dtRow)
                End With
            Next

            grd0.DataSource = dsMain.Tables(0)
            grd0.DataBind()
        Catch ex As Exception
        End Try
    End Sub
    ...
    Protected Sub grd0_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles grd0.SelectedIndexChanged
        Try
            Dim ItemID As String = grd0.SelectedRow.Cells(1).Text
