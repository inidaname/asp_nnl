...
Partial Class registerdevice
    ...
    Private Sub filldb(ByVal w As String)
        Dim ds As DataSet = GenTool.DataSetData("select sysID as ID,(select DeviceType from maindevicesection where sysID=deviceregistration.TypeOfDevice) as 'Instrument Type',actualMeasure as Capacity,modelNumber as Model,serialNumber as Serial,(select DeviceName from mainsubdevice where  mainsubdevice.sysID=DeviceGroupID) as Parameters,'0.00' as Fee,feeID as FID,BarCode,date_format(regDate,'%D-%M-%Y') as TransDate from deviceregistration where IsdbService=0 AND AccountID=" & Val(Session("ID")) & " " & w)
        Try
    ...
    Function getBill(EntryValue As String) As Double
        Dim sql As String = "Select * from maindeviceprice where MainDeviceID=" & Val(cboTypeOfDevice.SelectedValue & " order by MaxValue1 asc")
        Dim dsV As DataSet = GenTool.DataSetData(sql)
        Dim amtCalculated As Double = 0

        Try
            If GenTool.HasDatasetRecords(dsV) = True Then
                If dsV.Tables(0).Rows.Count = 1 Then
                    amtCalculated = Val(dsV.Tables(0).Rows(0).Item("Amount").ToString)
                Else

                    If Val(ddlUnitOfMeasure.SelectedValue) > 0 Then
                        Dim fs As String = "select sysID,MeasureValue,IsDefault from maindevicepriceadmeasure where MainDeviceID=" & Val(cboTypeOfDevice.SelectedValue) & " order by MeasureValue asc"
                        Dim dsB As DataSet = GenTool.DataSetData(fs)
                        Dim dsC As DataSet = GenTool.getFromDataset(dsB, "IsDefault=1 AND sysID=" & Val(ddlUnitOfMeasure.SelectedValue))

                        If GenTool.HasDatasetRecords(dsC) = False Then
                            dsC = GenTool.getFromDataset(dsB, "IsDefault=1")
                            If GenTool.HasDatasetRecords(dsC) = True Then
                                Dim MeasureValue As Double = Val(dsC.Tables(0).Rows(0).Item("MeasureValue").ToString)
                                dsC = GenTool.getFromDataset(dsB, "sysID=" & Val(ddlUnitOfMeasure.SelectedValue))
                                If GenTool.HasDatasetRecords(dsC) = True Then
                                    Dim MeasureValue1 As Double = Val(dsC.Tables(0).Rows(0).Item("MeasureValue").ToString)

                                    If MeasureValue1 < MeasureValue Then
                                        EntryValue = (MeasureValue1 / MeasureValue) * Val(EntryValue)
                                    Else
                                        EntryValue = MeasureValue1 / MeasureValue * Val(EntryValue)
                                    End If
                    ...
                    Dim dsA As DataSet = GenTool.DataSetData("Select * from maindeviceprice where MainDeviceID=" & Val(cboTypeOfDevice.SelectedValue) & " AND " & Val(EntryValue) & "<=MaxValue1 AND IsMaxValue=0")
                    For k As Integer = 0 To dsA.Tables(0).Rows.Count - 1
                        Dim g As String
                        g = dsA.Tables(0).Rows(k).Item("Amount").ToString
                    Next

                    If GenTool.HasDatasetRecords(dsA) = True Then
                        amtCalculated = Val(dsA.Tables(0).Rows(0).Item("Amount").ToString)
                    Else
                        dsA = GenTool.getFromDataset(dsV, "IsMaxValue=1")

                        Dim amt4First As Double = Val(dsA.Tables(0).Rows(0).Item("amt4First").ToString)
                        Dim amtAboveMaxValue As Double = Val(dsA.Tables(0).Rows(0).Item("amtAboveMaxValue").ToString)
                        Dim Value4Occurance As Double = Val(dsA.Tables(0).Rows(0).Item("Value4Occurance").ToString)
                        Dim Value4PartthereOf As Double = Val(dsA.Tables(0).Rows(0).Item("Value4PartthereOf").ToString)

                        If Value4PartthereOf = 0 Then
                            Value4PartthereOf = Value4Occurance
                        End If

                        Dim tsum As Double = amt4First
                        Dim rmValueSum As Double = Val(EntryValue) - Value4Occurance

                        Dim stsum As Int32 = (rmValueSum / Value4PartthereOf)
                        tsum += (stsum * amtAboveMaxValue)

                        If rmValueSum Mod Value4PartthereOf > 0 Then
                            tsum += amtAboveMaxValue
                        End If
    ...
    Private Sub saveData()
        ...
        Try
            ...
            If String.IsNullOrEmpty(Session("DeviceID")) = True Then
                ...
                txtTransCode.Text = GenTool.GenerateRNDCode(True, True) & GenTool.GenerateGlobalUniqueCode
Reapeat:        txtBarCode.Text = Strings.Left(GenTool.GenerateUniqueCode & GenTool.GenerateRNDCode, 8)
            ...
            txtamount.Text = getBill(txtActualMeasure.Text)

            If Val(txtamount.Text) = 0 Then
                msgtext = "Bill has not been set up for this instrument,please contact us"
                Return
            End If
            ...
            If String.IsNullOrEmpty(Session("DeviceID")) = True Then
                ...
                If GenTool.HasRows(sd) = True Then
                    msgtext = "Serial number already exist please choose another"
                    Return
                End If
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        Try
            ...
            Dim sd As String = "select sysID from paymentsheet where AmountPaid>0 AND companyID=" & Val(Session("ID")) & " AND TransCode=" & GenTool.addbackslash(txtTransCode.Text)
    ...
    Protected Sub cboMainGroup_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboMainGroup.SelectedIndexChanged
        Try
            ...
            txtActualMeasure.Visible = True
            txtActualMeasure.ValidationGroup = "actualMeasure|Invalid Official Measure"
            ddlUnitOfMeasure.Visible = True
            ...
            ElseIf cboMainGroup.SelectedItem.Text.ToUpper = "Banking".ToUpper Then
                ...
                txtActualMeasure.Visible = False
                txtActualMeasure.ValidationGroup = "actualMeasure"
                ddlUnitOfMeasure.Visible = False
            ...
            If cboMainGroup.SelectedItem.Text.ToUpper = "Oil & Gas".ToUpper Then
                cboProcessFluid.Visible = True
                Label29.Visible = True
            Else
                cboProcessFluid.Visible = False
    ...
    'select deviceregistration.sysID as SN,PaymentName as CompanyName,(select DeviceType from maindevicesection where mainsubdeviceID=TypeOfDevice limit 1) as InstrumentType,Location,modelNumber,serialNumber,amountDue as 'FEE Payable',AmountPaid,amountDue-AmountPaid as Balance,paymentsheet.regDate as TransDate from deviceregistration,paymentsheet  where paymentsheet.TransCode=deviceregistration.TransCode AND (narration='Annual Licensing Fee');
    ...
    Protected Sub btnBulkregistration_Click(sender As Object, e As System.EventArgs) Handles btnBulkregistration.Click
        Try
            Response.Redirect("bulkregisterdevice.aspx")
        Catch ex As Exception
        End Try
    End Sub

    Protected Sub cboTypeOfDevice_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles cboTypeOfDevice.SelectedIndexChanged
        Dim cDropDown As New dropDownFiller
        cDropDown.SQL = "select MeasureName,sysID from maindevicepriceadmeasure where Status=0 AND MainDeviceID=" & Val(cboTypeOfDevice.SelectedValue)
        cDropDown.ddlDown = ddlUnitOfMeasure
