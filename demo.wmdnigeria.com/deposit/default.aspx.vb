...
Public Class _default11
            ...
            If Not String.IsNullOrEmpty(Request.QueryString("OrderID")) And Not String.IsNullOrEmpty(Request.QueryString("TransactionReference")) Then
                ...
                If Not ReturnedOrderID = "" And Not TransactionReference = "" Then
                    Try
                        ...
                        If TransactionResponse.Contains("<br>") Then
                            TransactionResponse = TransactionResponse.Replace("<br>", ", ")
                        End If
                    ...
                    Finally
                        ...
                        If TransactionResponse = "There is no row at position 0." Then
                            PaymentResponse.Text = "Invalid transaction, please try again!"
                        ...
                        Else
                            ...
                            Try
                                ...
                                If Status = "Successful" Then
                                    Response.Redirect("success/?TransactionReference=" + TransactionReference + "&OrderID=" + ReturnedOrderID)

                                ElseIf Status = "Failed" Then
                                    PaymentResponse.Text = "Transaction failed, please try again!"

                                ElseIf Status = "Not Processed" Then
                                    PaymentResponse.Text = "Transaction could not be processed, please try again!"
                                    
                                ElseIf Status = "NotProcessed" Then
                                    PaymentResponse.Text = "Transaction could not be processed, please try again!"
                                    
                                ElseIf Status = "Pending" Then
                                    PaymentResponse.Text = "Transaction pending, please check back later!"

                                ElseIf Status = "Cancelled" Then
                                    PaymentResponse.Text = "This transaction has been cancelled!"
                                Else

                                    PaymentResponse.Text = Status
                                End If
    ...
    Protected Sub SaveDeposit_Click(sender As Object, e As EventArgs) Handles SaveDeposit.Click
        ...
        If DepositAmount.Text = "" Or DepositPaymentType.SelectedItem.Text.Contains("Select") Then
        ...
        Else
            ...
            If DepositPaymentType.SelectedValue = "Online" Then
                ...
                Try
                    ...
                    Dim com As New MySqlCommand
                    com.Connection = conn
                    com.CommandText = "INSERT INTO deposits (companyID,amountDeposit,narration,paymentMode,depositDate,depositTime,confirmed,orderID,companyName) VALUE ('" & Session("UserLoginID") & "','" & DepositAmount.Text & "','" & DepositNarration.Text & "','" & DepositPaymentType.SelectedValue & "','" & TodaysDate() & "','" & CurrentTim() & "','" & confirmed & "','" & OrderIDText.Text & "','" & Session("UserLoginCompanyName") & "')"
                ...
                Catch ex As Exception
                    PaymentResponse.Text = ex.Message
                Finally
                    DisconnectDatabase()
            ...
            Else
                ...
                If Not DepositAmount.Text = "" Or DepositPaymentType.SelectedItem.Text.Contains("Select") Or BankName.SelectedItem.Text.Contains("Select") Then
                    Try
                        ...
                        Dim com As New MySqlCommand
                        com.Connection = conn
                        com.CommandText = "INSERT INTO deposits (companyID,amountDeposit,narration,paymentMode,depositDate,depositTime,confirmed,bankName,orderID,companyName) VALUE ('" & Session("UserLoginID") & "','" & DepositAmount.Text & "','" & DepositNarration.Text & "','" & DepositPaymentType.SelectedValue & "','" & TodaysDate() & "','" & CurrentTim() & "','" & confirmed & "','" & BankName.SelectedItem.Text & "','" & OrderIDText.Text & "','" & Session("UserLoginCompanyName") & "')"
                        'com.CommandText = "INSERT INTO deposits (companyID,transCode,amountDue,invoiceDate,invoiceTime,narration,orderId) VALUE ('" & Session("UserLoginID") & "','" & UniqueID() & "','" & DepositAmount.Text & "','" & TodaysDate() & "','" & CurrentTim() & "', '" & DepositNarration.Text & "','" & OrderID() & "')"
                        com.ExecuteNonQuery()

                        PaymentResponse.Text = DepositAmount.Text & " has been deposited to your account successfully, awaiting Administrator approval!"
                        Try
                            DepositAmount.Text = FormatNumber(DepositAmount.Text, 2, TriState.False, , TriState.True)
                        Catch ex As Exception
                            PaymentResponse.Text = "Only digits and/or a decimal please."
                        End Try
                        Session.Remove("AmountPaid")
                        Session.Add("AmountPaid", DepositAmount.Text)
                        'Send SMS Message
                        tools.SendSMS(Me, GetSMSAPI(), GetSMSUsername(), GetSMSPassword(), BulkSMSDefaultSender(), SMSRecipientNumber(), CashWalletMessage())
                        ...
                        Dim Activity As String = Session("UserLoginName") & " deposit  " & DepositAmount.Text & " to " & BankName.SelectedItem.Text & " for: " & DepositNarration.Text
                    ...
                    End Try
                Else
                    PaymentResponse.Text = "Select a valid option and make sure the Amount field is not blank!"
    ...
    Protected Sub FilterDepositButton_Click(sender As Object, e As EventArgs) Handles FilterDepositButton.Click
        ...
        If FilterDeposit.SelectedValue = "Date" Then
            ...
            Else
                Dim DateDifference
                Dim FormatStartDate = Date.ParseExact(DateFrom.Text, "dd-MM-yyyy", Nothing)
                Dim FormatEndDate = Date.ParseExact(DateTo.Text, "dd-MM-yyyy", Nothing)
                Dim DateFormat As String = "dd-MM-yyyy"
                Dim StartDate = FormatStartDate.ToString(DateFormat.ToString)
                Dim EndDate = FormatEndDate.ToString(DateFormat.ToString)

                DateDifference = Date.ParseExact(DateTo.Text, "dd-MM-yyyy", Nothing) - Date.ParseExact(DateFrom.Text, "dd-MM-yyyy", Nothing)
