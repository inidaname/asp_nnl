...
Public Class _default
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
        'MessageBox.Show(Me, Session("UserLoginUserType"))
    ...
    Protected Sub SearchAccount_Click(sender As Object, e As EventArgs) Handles SearchAcount.Click
        Session.Add("UserTryingToResetPasswordEmail", Account.Text)
        ErrorMessage.Text = ""
                ...
                Else
                    ...
                    Using cn As MySqlCommand = New MySqlCommand(db, conn)
                        Using da As New MySqlDataAdapter(cn)
                            ...
                            Session.Add("UserTryingToResetPassword", ds.Tables(0).Rows(0).Item("ID").ToString)
                            Session.Add("UserTryingToResetPasswordNumber", ds.Tables(0).Rows(0).Item("companyPhoneNumber").ToString)
                            Session.Add("UserTryingToResetPasswordMobile", ds.Tables(0).Rows(0).Item("filledByMobile").ToString)
                            Session.Add("UserLoginCompanyPhoneNumber", ds.Tables(0).Rows(0).Item("companyPhoneNumber").ToString)
                            Session.Add("UserLoginCompanyFilledByMobile", ds.Tables(0).Rows(0).Item("filledByMobile").ToString)
                            Session.Add("UserTryingToResetPasswordUsername", ds.Tables(0).Rows(0).Item("username").ToString)
                            Session.Add("UserTryingToResetPasswordCompany", ds.Tables(0).Rows(0).Item("companyName").ToString)
    ...
    Protected Sub ResendPasswordResetCode_Click(sender As Object, e As EventArgs) Handles ResendPasswordResetCode.Click
        ...
        Try
            ...
            Dim dbc As String = "SELECT * FROM company WHERE companyEmail='" & AccountEmail.Text & "';"
            Using cnc As MySqlCommand = New MySqlCommand(dbc, conn)
                Using dac As New MySqlDataAdapter(cnc)
                    ...
                    If PasswordRequestChecker.HasRows = 0 Then
                        ...
                        ErrorMessage.Visible = True
                        ErrorMessage.Text = "Sorry, password reset code not found, try again in 24 hours!. Thanks you!"
                        ErrorMessage.ForeColor = Drawing.Color.DarkRed
                    ...
                    Else
                        ...
                        Using cn As MySqlCommand = New MySqlCommand(db, conn)
                            Using da As New MySqlDataAdapter(cn)
                                ...
                                ErrorMessage.Visible = True
                                ErrorMessage.Text = "Password reset code has been resend successfully!"
            ...
            Using cn As MySqlCommand = New MySqlCommand(db, conn)
                Using da As New MySqlDataAdapter(cn)
                    ...
                    If PasswordResetCode.Text = Session("PasswordResetCode") Then
                        ...
                        ResendPasswordResetCode.Visible = False
